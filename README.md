...
### live-mode
![my-ime-eager-mode demo](demo3.gif)
...
変換 worker は 3 つの操作を持ちます。stdio transport では JSON Lines の `method` として呼び、互換用 HTTP server では同名の endpoint として呼べます。
...
## 互換 HTTP server を直接起動

Emacs の通常利用では不要ですが、HTTP API を curl で確認したい場合や別 client から使いたい場合は、互換 HTTP server を直接起動できます。Docker を使わない場合は、手元の `PATH` 上に `kkc` コマンドが必要です。
...
macOS のライブ変換に近い使い方を試す場合は、`my-ime-live-mode` を使います。
入力中の範囲はローマ字のまま保持し、見た目だけ overlay で変換候補に差し替えます。
`RET` は見えている live 候補をそのまま確定して改行します。
...
```text
RET       live 候補を確定して改行
C-o       現在の live 範囲で候補選択
C-c j RET live 候補を確定
C-c j l   live-mode の切り替え
```

デフォルトでは `/convert` を使って漢字候補まで表示します。軽いかな preview だけにしたい場合:
...
変換 worker は起動時にプロジェクト内の `data/*.skk` を自動で読みます。標準で `data/my-ime-tech.skk` を同梱しており、技術文でよく使うカタカナ語や短い実用語を 2394 エントリ入れています。
...
通常の Emacs 利用では stdio worker を使うため HTTP server は不要です。この API は互換用、curl での確認用、別 client から使う場合の入口です。起動するには `make run` を使います。
...
テストでは stdio worker、互換用 HTTP handler、Emacs client、用語保護と復元、辞書、ローマ字変換、preedit、kkc バックエンドを確認します。
